...
-- Allow teachers to see student profiles for their subjects
create policy "teachers_view_students"
  on public.profiles for select
  using (
    role = 'student' and exists (
      select 1 from public.enrollments e
      inner join public.subjects s on s.id = e.subject_id
...
create policy "students_view_enrolled_subjects"
  on public.subjects for select
  using (
    exists (
